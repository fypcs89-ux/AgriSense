...
minmax_scaler = pickle.load(open('model/fertminmaxscaler.pkl', 'rb'))
standard_scaler = pickle.load(open('model/fertstandardscaler.pkl', 'rb'))
...
def predict():
    try:
        ...
        # Combine input
        input_data = np.array([[N, P, K, temp, soil_type, pH, crop_type]])

        # Apply scalers
        input_scaled = minmax_scaler.transform(input_data)
        input_final = standard_scaler.transform(input_scaled)

        # Predict
        prediction = model.predict(input_final)
        predicted_fertilizer = fertilizer_map_reverse[int(prediction[0])]
